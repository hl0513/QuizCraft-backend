...
import com.hhj.quizCraft.manager.AiManager;
import com.hhj.quizCraft.model.dto.question.*;
...
import com.hhj.quizCraft.service.QuestionService;
...
public class QuestionController {
    ...
    // region AI 生成题目功能
    public static final String GENERATE_QUESTION_SYSTEM_MESSAGE = "你是一位严谨的判题专家，我会给你如下信息：\n" +
            "```\n" +
            "应用名称，\n" +
            "【【【应用描述】】】，\n" +
            "应用类别,\n" +
            ...
            "```\n" +
            "title是题目,options是选项,每个选项的key按照英文字母序(比如A、B、C、D)以此类推,value是选项内容\n" +
    ...
    public SseEmitter aiGenerateQuestionSSE(AiGenerateQuestionRequest aiGenerateQuestionRequest,HttpServletRequest request) {
        ...
        // 左括号计数器，除了默认值外，当回归为 0 时，表示左括号等于右括号，可以截取结果
        ...
        modelDataFlowable
                ...
                .flatMap(message -> {
                    ...
                    return Flowable.fromIterable(charactersList);
                })
                .doOnNext(c-> {
                    // 如果是 '{' , 则加一
                    if(c == '{'){
